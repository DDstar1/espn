...
from page_commentary.get_commentary_details import get_details_and_commentary_of_game
...
chrome_options = Options()
 # Quiet logs and automation suppression
...
chrome_options.add_argument("--no-sandbox")  # Recommended in Docker or restricted environments
chrome_options.add_argument("--disable-dev-shm-usage")  # Helps avoid resource issues
chrome_options.add_argument("--blink-settings=imagesEnabled=false")  # Disable images for faster load
...
def get_links_of_all_games_played(team_url):
    #Update the latest scraped team url to processing
    ...
    for game_detail_url in game_detail_Links:
        ...
        print(goals[0].get_attribute('innerHTML'))
        ...
        for i, logo in enumerate(logos):
            ...
            if  team_espn_id == None:
                print(f'Skipping team with missing espn_id for {team_name}')
                continue
            ...
            # Goals extraction (added check incase of svg or other non-text nodes)
            goals_text = driver.execute_script("return arguments[0].childNodes[0].nodeValue.trim();", goals[i])
            print(goals_text)
            print("Goals")
