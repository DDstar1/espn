...
def insert_team(data):
    ...
    try:
        ...
        my_print(f"Team with espn_id {data['espn_id']} already exists.")
    except Exception as e:
        my_print("An error occurred with data", data, e)
    finally:
        conn.close()
...
def team_exists(espn_id):
    ...
    if exists == True: my_print(f"Team with espn_id {espn_id} already exists..")
    conn.close()
    return exists
...
def game_info_exists(espn_id):
    ...
    exists = cursor.fetchone() is not None
    if exists == True: my_print(f"Game Info with espn_id {espn_id} already exists.")
    conn.close()
    return exists
...
def insert_line_up_statistics(data):
    ...
    cursor.execute("""
        INSERT OR IGNORE INTO Line_Up_Statistics (
            ...
            fouls_commited, fouls_suffered, assists, offsides,
...
def set_latest_scraped_team_url(url):
    """Insert a new scraped team URL into the Tracker table."""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("""
        INSERT INTO Tracker (scraped_team_url)
        VALUES (?)
    """, (url,))
    conn.commit()
    conn.close()
...
def get_latest_scraped_team_url():
    """
    Return the oldest 'processing' team URL scraped more than 30 minutes ago.
    If none are found, return the most recently 'done' team URL.
    """
    conn = get_db_connection()
    cursor = conn.cursor()

    # Step 1: Get oldest "processing" row older than 30 minutes
    cursor.execute("""
        SELECT scraped_team_url
        FROM tracker
        WHERE status = 'processing'
          AND scraped_at <= datetime('now', '-30 minutes')
    ...
    # Step 2: Fallback to most recent "done" row
    cursor.execute("""
        SELECT scraped_team_url
        FROM tracker
        WHERE status = 'done'
